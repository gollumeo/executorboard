...
- Architecture: Domain-driven layering (Domain → Application → Persistence → Infrastructure → Presentation/API). Controllers and endpoints are not implemented yet.
- Domain: Estates and Auth aggregates with value objects (`EstateId`, `ExecutorId`, `EstateName`, `ParticipantId`, `InvitationToken`, `Email`, `PasswordHash`, `SessionToken`, etc.).
...
- Persistence: EF Core 10 with PostgreSQL and SQLite providers. `ExecutorBoardDbContext` maps `Users` and `Estates`; repository implementations fulfill the application ports.
- Tests: xUnit suite in `ExecutorBoard.Tests` covering domain invariants, application flows, and EF repositories.
  - Run: `dotnet test apps/api/ExecutorBoard/ExecutorBoard.Tests/ExecutorBoard.Tests.csproj`
...
- Scripts: `pnpm dev`, `pnpm build`, `pnpm preview`, `pnpm lint` (see `apps/landing/package.json`).

## Getting started
- Prereqs: .NET 10 SDK (preview/RC), Node.js 18+, pnpm.
- Backend: `cd apps/api/ExecutorBoard && dotnet restore` (the API host is scaffolded but has no public endpoints yet).
